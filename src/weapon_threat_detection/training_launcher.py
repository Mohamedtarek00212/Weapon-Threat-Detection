...
def _validate_configuration(context: LauncherContext) -> None:
    ...
    if training["image_size"] != 800 or training["batch_size"] != 20:
        raise ValueError("The approved configuration requires image_size=800 and batch_size=20")
...
def smoke_test(root: str | Path | None = None) -> dict[str, Any]:
    ...
    print("training.device =", training["device"])
    print("detected_hardware.device =", detected_hardware.device)
    print("expected_device =", expected_device)
    ...
    result = {
        ...
        "auto_device_selection": (
            ...
            and str(detected_hardware.device).startswith("cuda")
            and str(trainer.args.device) in ("0", "cuda:0")
        ),
